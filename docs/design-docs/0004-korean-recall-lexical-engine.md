...
# 한국어 Recall의 어휘 검색은 Go에서 형태소로 나눈 뒤 FTS5에 넣는다
...
**Go 코드에서 한국어 형태소로 나눈 뒤, 공백으로 이어 붙여 FTS5
(`unicode61`) 인덱스에 저장하고 BM25 로 랭킹한다.** 색인과 질의에 동일한
`Tokenizer` 를 쓴다.

- 형태소 분석기는 `memories.Tokenizer` 인터페이스 뒤에 둔다. 기본 구현은 순수 Go 인 **Kagome v2 + kagome-dict-ko**
  (`internal/tokenize`), cgo 없음.
- `internal/migrate` 가 GORM schema apply 뒤 `memories_fts` FTS5 가상 테이블을 만든다.
  토큰화는 Go 에서 하므로 SQL은 빈 테이블만 만들고, Go 코드의 재색인이
  기존 Memory 를 백필한다.
- 쓰기 경로(`ReplaceSourceMemories`)가 Memory 저장과 같은 트랜잭션에서 FTS 를
  동기화한다. Scope 필터는 `memories_fts → memory_links → sources.scope_value`
  조인으로 구현한다.
- 재사용 가능한 검색 진입점은 `memories.Recaller.Search(query, scope, limit)` 이며,
...
- FTS5 커스텀 토크나이저는 **C 레벨 전용**이라 pure-Go 인 `modernc.org/sqlite`
  로는 등록할 수 없다. Go 코드에서 먼저 토큰화하면 SQLite 토크나이저 API 를 건드리지 않으므로
  **modernc 유지**가 가능하다(mattn/go-sqlite3 cgo 전환 불필요).
- 형태소 분석은 조사·어미를 분리해(`종목을` → `종목`+`을`) 조사 없는 명사 질의가
...
- 의미 검색(임베딩)·하이브리드(BM25+dense RRF)·sqlite-vec 는 같은 `Search` 계약
  위에 나중에 덧붙인다(로드맵). 이 ADR 은 어휘 층만 확정한다.

## Considered options

- **Go 코드 형태소 + FTS5 (선택)** — modernc 유지, cgo 0, 한국어 재현율 확보.
