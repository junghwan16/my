...
218개 세션으로 직접 써 본 결과 두 랭커의 top-5 겹침이 0~2/5 에 불과했다 — 같은 질의에
...
진입점 `Recaller.Recall` 이 단일 하이브리드 랭킹을 반환하게 한다. 의미 엔진이 있으면
...
- **RRF 융합**: Memory ID 기준으로 `score(m) = Σ 1/(k + rank_i(m))` 를 각 랭킹
  ...
  살린다.
- **하이브리드 진입점**: 새 `Store.HybridRecallResults` 가 `SearchMemories` 와
  `SearchSemantic` 을 각각 넉넉히(overfetch=50) 호출해 융합 풀을 만들고, RRF 로
  랭킹한 뒤 limit 로 자른다. Source 컨텍스트는 기존 `attachSources` 를 그대로 재사용해
  결과를 `[]RecallResult` 으로 맞춘다. `Recall` 은 task 가 있을 때 이 메서드를
  호출한다 — 시그니처·반환 구조 불변이라 CLI(`gieok memory recall`)와 MCP `recall`
  툴이 호출부 수정 없이 개선된 랭킹만 본다.
- **자동 강등**: 임베더가 없으면(Ollama 미가용) `SearchSemantic` 이 빈 리스트를
  반환하므로(ADR-0005 계약), 융합은 어휘 랭킹 하나만 남아 사실상 어휘 전용 순서로
  동작한다 — 에러도, 계약 변경도 없다. 완전 오프라인 기본 빌드가 그대로 유지된다.
- **동점 처리**: 융합 점수가 같으면 최신 `CreatedAt`, 그다음 `MemoryID`
  오름차순으로 깨뜨린다(의미 랭커의 동점 처리와 동일). 같은 입력에 항상 같은 순서.
- **빈 task 경로 불변**: task 가 비면 여전히 `RecentRecallResults`(Scope 안 최신
...
- **왜 RRF**: 점수 정규화 없이 rank 만으로 서로 다른 스케일(BM25 점수 vs 코사인
  ...
  하이브리드 검색의 사실상 표준이다. 실제 사용에서 나온 낮은 겹침(0~2/5)이 딱 RRF 가
  강한 상황 — 보완적 랭킹의 융합.
- **왜 공유 진입점에서**: `Recall` 이 CLI·MCP 의 단일 진입점이므로, 융합을 이 안에
  넣으면 CLI와 MCP가 함께 개선된다. 계약(시그니처·`[]RecallResult` 구조)이
  그대로라 호출부 수정이 0.
...
- 의미 엔진이 붙은 환경에서는 recall 결과에 어휘상 매칭되지 않지만 의미상 가까운
  ...
  전혀 없으면 두 랭커 모두 비어 "no matching memory" 가 그대로 유지된다.
...
- **어휘 우선 + 의미 보조 (거절)** — 어휘가 결과를 내면 의미를 안 쓰므로,
  실제 사용에서 확인한 보완 효과(의미만 떠올리는 Memory)를 놓친다.
