...
`WorkspaceKey`(순수 함수, `internal/source/scope.go`)를 도입하고, scoped Recall 을
key 기반으로 만들었다. `Recaller.Recall` 은 scope 가 비면 종전대로 전역 랭킹을 내고,
scope 가 지정되면 **전 scope 를 랭킹(overfetch)한 뒤 그 Source 의 WorkspaceKey 가 질의
scope 의 key 와 일치하는 결과만** 상위 limit 개 남긴다. 랭킹은 store 의 전역 순서 그대로라
재정렬이 아니라 선택이다. 스키마 변경 없음 — 되돌릴 수 있다.

### 측정 (2026-07-02, snapshot 566 memories)

- 정본 `/personal/gieok` scope recall: **0 → 3 결과** (질의 지식이 `/my` 에 있어도 정본
  경로에서 회수됨).
- eval hit@5(hybrid): **3/3(100%) 유지**, MRR **0.833 → 1.000** (pooling 으로 최적
  메모리가 rank 1 로 상승). 정본 scope 로 시나리오를 돌려도 3/3.

## 한계와 2단계 (store-side scope_key)

Recaller overfetch(기본 100, limit×10)는 로컬 규모(수천 메모리)에서 충분하지만, in-key
hit 가 전역 상위 overfetch 밖으로 밀리면 놓칠 수 있다 — scale 에서의 근사다. 원칙적
해법은 `sources.scope_key` 컬럼(write 시 `WorkspaceKey` 로 채우고, 멱등 마이그레이션으로
backfill)을 두고 store 필터를 `sr.scope_value = ?` 에서 `sr.scope_key = ?` 로 바꾸는
것이다. 그러면 overfetch 없이 SQL 이 정확히 필터한다. 필요해질 때(코퍼스가 수만 규모로
커질 때) 2단계로 진행한다. GitHub issue #21 과 연결.

`DefaultScopeAliases` 의 rename 표는 지금 하드코딩(`personal/my`→`personal/gieok`)이며,
설정화는 후속 과제다.
