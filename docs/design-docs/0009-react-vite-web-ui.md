...
- **오프라인 불변식 유지**: Tailwind 는 빌드 타임에 정적 CSS 로 컴파일되고, shadcn 컴포넌트는
  ...
  번들해 woff2 가 빌드 자산이 되고, 한글은 시스템 폰트 스택으로 둔다. 그래프 라이브러리
  (react-force-graph-2d)도 CDN vendoring 대신 번들에 포함해 자체 완결이다 → 런타임·CDN
  의존 0.
...
- **왜 빌드 프레임워크**: 두 페이지 + 상태 + 편집 + 재디자인을 vanilla 로 이어가면
  유지보수 비용이 크다. React + TS 가 컴포넌트·타입·상태를 정리하고, 그래프도
  React 컴포넌트로 감싸 재사용한다. 그래프는 Obsidian 처럼 연속 force 시뮬레이션
  (드래그 시 이웃이 딸려오고 스프링처럼 안정화)이 필요해 react-force-graph-2d(d3-force
  기반)를 쓴다 — 정적 레이아웃만 내는 cytoscape 에서 갈아탔다.
