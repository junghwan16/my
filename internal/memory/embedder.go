...
// Tokenizer seam, the concrete model lives behind this interface so it can be
...
// error when Ollama is unreachable, so construction can fall back gracefully.
type Embedder interface {
	...
	// so a model change is detectable and stale vectors can be re-embedded.
